...
class ResultinfoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request['id'] == "all"){
            $result = Resultinfo::all();
            return $this->sendResponse($result, 'Resultinfos list ');

        }else if($request['id']){
            $result = Resultinfo::findOrFail($request['id']);
            return $this->sendResponse($result, 'Resultinfos ');
        }
        ...
        if($sortField)
            $resultinfos = $query->orderBy($sortField, $sortType)->paginate( $perPage );
        else       
            $resultinfos = $query->paginate( $perPage );
  
        return $this->sendResponse($resultinfos, 'Resultinfos list ');
    }
    ...
    public function search($request, $query)
    {
        ...
        $fields = Schema::getColumnListing('$resultinfos');
    ...
    public function store(Request $request)
    {
        ...
        if($model_id && $relation &&  $pv_ids){ // Many to Many relationships
            ...
            $resultinfo = Resultinfo::create($request->all());    
            return $this->sendResponse( $resultinfo, 'Resultinfo Created Successfully');
        }
    ...
    public function checkValidation(Request $request){
        $data = DB::select('DESCRIBE '.strtolower( 'Resultinfos' ));
    ...
    {
        $this->checkValidation($request);
        $resultinfo = Resultinfo::findOrFail($id);
        $input = $request->all();
        $resultinfo->fill($input)->save();
        return $this->sendResponse($resultinfo, 'Resultinfo Information has been updated');
    }
    ...
    public function destroy($parameters)
    {
        ...
        if (array_key_exists('tbl', $data) && array_key_exists('pv_tbl', $data) 
           ...
           { // Many to Many relationships
            ...
            Resultinfo::whereIn('id', $idsArray)->delete();
