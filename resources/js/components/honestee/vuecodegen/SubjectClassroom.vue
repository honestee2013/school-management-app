                ...
                <div class="card-body table-responsive p-2"> 
                    <div class="form-row mb-2">
                        <div v-show="queryId" class="bg-light py-2 col-12 text-bold mb-2 pl-3" style="margin-top:-0.2em">
                            Subjects for <h5> {{selectedName}} </h5>
                        </div>
                        ...
                        <div class="col">
                            <select class="custom-select" v-model="queryId" @change="onQueryIdChanged($event, 'classroom' )">
                              ...
                              <option v-if= "classrooms" 
                                      v-for = "row in classrooms"  
                      ...
                      :columns="columns">
                                  ...
                                  <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                    <button href="#" class="dropdown-item" @click.prevent="print">
                                      ...
                                      <button href="#" class="dropdown-item">
                                        ...
                                        <json-excel class="" :data="subjects" :fields="table_heders" worksheet="subject List" name="subject_lists.xls">
                            ...
                            <span v-if="props.column.field == 'action'">
                              <div class="btn-group">
                                ...
                                <div class="dropdown-menu">
                                  ...
                                  <a class="dropdown-item " href="#" @click="deleteClassroomSubjects(props.row.id)"><i class="fa fa-trash">  <span style="margin-left:0.1em"> Remove </span>  </i></a>
                                </div>
                              </div>
                            </span>
                            <span v-else>
                          ...
                          <div slot="selected-row-actions">
                              <div class="dropdown">
                                  ...
                                  <div class="dropdown-menu">
                                    <a class="dropdown-item " href="#" @click="deleteSelectedSubjects()"><i class="fa fa-trash">  <span style="margin-left:0.1em"> Remove </span>  </i></a>
                                  </div>
                                </div>
                          </div>
                    </vue-good-table>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header"> <!-- Modal Header -->
                      <h5 class="modal-title" v-show="!editmode">Add subjects</h5>
                      <h5 class="modal-title" v-show="editmode">Update subjects</h5>
                  ...
                  <form @submit.prevent="editmode ? updateClassroomSubjects() : createClassroomSubjects()">
                    <div class="modal-body">

                        <div class="card border-light w-100" >
                          <div class="card-header">
                            Select subjects for <h5> {{selectedName}} </h5>
                          </div>
                          <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item bg-light">
                              <div class="form-check">
                                ...
                                <label class="form-check-label" for="select_all" > All subjects </label>
                              </div>
                            </li> 
                          
                          
                          
                            <li v-for="row in subjects" class="list-group-item">
                              <div class="form-check">
                                <input class="form-check-input" type="checkbox" :value="row.id" :id="row.id" v-model="checkedSubjects">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        data-dismiss="modal" 
                        @click="deleteClassroomSubjects(clickedRow.id)"><i class="fa fa-trash"></i> Remove </button>
    ...
    export default {
        ...
        methods: {
            ...
            updateClassroomSubjects(){
                ...
                this.form.put("api/subject-classrooms/"+this.form.id)
                .then((response) => {
                    // success
                    $('#addNew').modal('hide');
                    Toast.fire({
                    ...
                    confirmButtonText: 'Yes, delete it!'
                        ...
                        // Send request to the server
                         if (result.value) {
                              ...
                              parameters["tbl"] = "subject";
                              parameters["pv_tbl"] = this.queryTable;
                              parameters["pv_id"] = this.queryId;
                              parameters["pv_ids"] = id;//this.checkedSubjects;
                         
                                //const theData = [id];
                                axios.delete('api/subject-classrooms/'+JSON.stringify(parameters) ).then(()=>{
                                        Swal.fire(
                                        'Deleted!',
                    ...
                    confirmButtonText: 'Yes, delete it!'
                        ...
                        // Send request to the server
                         if (result.value) {
                              ...
                              parameters["pv_tbl"] = this.queryTable;
                              parameters["pv_id"] = this.queryId;
                              parameters["pv_ids"] = this.selectedRows;//this.checkedSubjects;
                         


                                let theData = JSON.stringify( parameters );
                                axios.delete("api/subject-classrooms/"+theData).then(()=>{
                                        Swal.fire(
                                        'Deleted!',
                                        'The subject was deleted successfully.',
                                        'success'
            ...
            createClassroomSubjects(){
                var parameters = "?tbl=subject";
                parameters = parameters + "&pv_tbl="+this.queryTable;
                parameters = parameters + "&pv_id="+this.queryId;
                parameters = parameters + "&pv_ids="+this.checkedSubjects;
                var url = "api/subject-classrooms"+parameters;
            ...
            onSelectAll(){
              ...
              if(this.selectAll){
                  var ids = this.subjects
                    .map(function (data) { return data.id; });  
                  this.checkedSubjects = ids;
              } else {
            ...
            loadClassroomSubjects() {
                ...
                parameters = parameters + "&tbl=subject";
                parameters = parameters + "&pv_tbl="+this.queryTable;
                parameters = parameters + "&pv_id="+this.queryId;
                var url = "api/subject-classrooms"+parameters;
                ...
                try{
                    this.form.get( url ).then( subjects  => {
                        if(subjects.data.data){
                          ...
                          this.classroomSubjects = subjects.data.data.data;
                          this.checkedSubjects 
