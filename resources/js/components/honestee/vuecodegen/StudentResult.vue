<template>
  ...
  <nav class="navbar navbar-light bg-light">
        <form class="form-inline pt-3 pt-sm-0"  @submit.prevent="editResultMode ? editResult() : getResult()">
          
          <select v-model="result_scope" class="custom-select m-sm-2 my-2" required = "required" @changed="( (isSettings)? showSettings(): '' )">
          ...
          <input v-show="(result_scope == 'Student')"  v-model="user_number" type="text" class="form-control" required = "required" placeholder="User Number" >
          
          <select v-model="resultYear" class="custom-select m-sm-2 my-2" required = "required" v-show="(result_scope == 'Student'|| result_scope == 'School' || result_scope == 'class') ">
          ...
          <select  v-model="resultTerm"  class="custom-select mr-sm-2" required = "required" v-show="(result_scope == 'Student'|| result_scope == 'School' || result_scope == 'class')">
          ...
          <button type="submit" class="btn btn-primary my-sm-2 my-3" v-if="(result_scope == 'Student')" @click="editResultMode = isSettings = false">Check</button>
          <button type="submit" class="btn btn-primary my-sm-2 ml-2 my-3" v-show="(!editResultMode) && ((user && result_scope == 'Student') || isSettings)"  @click="editResultMode = true"> Edit</button>
    ...
    <div class="container table-responsive py-3" style="background:lightgray; width:100% " v-show="(isSettings)" >
      <form>   
          <div class="card ">
              <div class="card-header">
                  <h4> {{ result_scope }} result settings </h4>
              </div>
              <div class="card-body" >
                  <div class="my-3" v-show="result_scope=='School'">
                      <h5 class="card-title">School resumption date</h5><br />
                      <p class="card-text"> This date will be added on each student result automatically</p>
                      <div class="form-group">
                        <label for="formGroupExampleInput">Resumption date</label>
                        <input type="date" class="form-control" id="formGroupExampleInput" placeholder="Next term begins on"  :disabled="!editResultMode" v-model="next_term_begins_on">
                      </div>
                      <hr />
                  </div> 
                  ...
                  <div class="mt-5">
                    <h5 v-show="result_scope=='School'" class="card-title">Holding/Releasing the whole school result</h5><br />
                    <h5 v-show="result_scope=='Student'" class="card-title">Holding/Releasing the student result</h5><br />

                    <div class="form-group">
                        <div class="custom-control custom-checkbox card-text">
                          <input type="checkbox" class="custom-control-input " id="customCheck1" :disabled="!editResultMode" v-model="hold_result" /> 
                          <label class="custom-control-label" for="customCheck1">Hold this {{ result_scope.toLowerCase() }} result</label>
                        </div>
                    </div>
                    <div class="form-group">
                      <label for="exampleFormControlTextarea1">Reason for holding the {{ result_scope.toLowerCase() }} result</label>
                      <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" :disabled="!editResultMode" v-model="reason_for_holding_result"></textarea>
                    </div>
                  </div>
                  <hr />
              ...
              <div class="row mb-2" style=" height:auto; width:96.7% ; margin:auto; padding:1%;">
                ...
                <div class="mx-1" style="width:55%; text-align:center">
                  <p> 
                    ...
                    <!--<span> Motto: Some school motto </span>  <br />-->
                    <span v-show="assessments.schoolAddress"> {{ assessments.schoolAddress }} </span> <br />
                    <span v-show="assessments.schoolPhone"> {{ assessments.schoolPhone + "; " + assessments.schoolEmail }} </span> 
                  </p> 
  ...
  export default {
    ...
    methods: {
      ...
      saveResult(){
        ...
        //if(this.isSettings){
            //this.saveSettings();
        parameters = parameters + "&result_scope=" + this.result_scope;
        parameters = parameters + "&hold_result=" + this.hold_result;
        parameters = parameters + "&reason_for_holding_result=" + this.reason_for_holding_result;
        parameters = parameters + "&next_term_begins_on=" + this.next_term_begins_on;



        //}
        if(!this.user && this.result_scope == "Student") // Nothing to save
          return;



        parameters = parameters + "&user_id=" + 0;
        parameters = parameters + "&user_number=" + this.user_number;
      ...
      showSettings(){
        ...
        //this.assessments = [];
        //alert(this.isSettings);

        this.getResult();

        if (this.resultYear && this.resultTerm && this.result_scope == "Student" && (Object.keys(this.assessments).length > 0) ) 
            this.isSettings = true;
        else if (this.resultYear && this.resultTerm && this.result_scope == "School") 
            this.isSettings = true;
        else
            this.isSettings = false;


        /*if(this.result_scope && this.resultYear && this.resultTerm){
      ...
      getResult(){
          //console.log( this.user_number+" "+ this.resultTerm+" "+this.resultYear );
          //this.isSettings = false;
          ...
          try {
            ...
            axios.get(url).then(user => {
              if (user.data.data) {
                ...
                //alert(JSON.stringify( this.user.id));
                    ...
                    if(!this.isSettings){
                      ...
                      //alert(JSON.stringify( this.assessments ));
                        ...
                        if(assessments.data.data.studentResultInfo.length){
                          ...
                          this.hold_result = assessments.data.data.studentResultInfo[0].hold_result;
                          this.reason_for_holding_result = assessments.data.data.studentResultInfo[0].reason_for_holding_result;
                        }

                    } else { // Settings 

                        if(assessments.data.data.length < 1){
                          Swal.fire({
                            position: 'top-end',
                            icon: 'error',
                          ...
                          this.hold_result = assessments.data.data[0].hold_result;
                          this.reason_for_holding_result = assessments.data.data[0].reason_for_holding_result;
                          this.next_term_begins_on = assessments.data.data[0].next_term_begins_on;
                        }
